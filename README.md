...
```bash
npm install -g mermaid-er-viewer
```

インストール後、どのディレクトリからでも実行できます：

```bash
mermaid-er-viewer path/to/your-diagram.mmd
```

### npx で一時実行（インストール不要）

```bash
npx mermaid-er-viewer path/to/your-diagram.mmd
```

### Claude Code の MCP サーバーとして使う

Claude Code から ER 図のパース・レイアウト操作・ビューワー起動などをツールとして利用できます。

Claude Code に以下をコピペして指示するだけで設定できます：

```
MCPサーバーを追加して。名前: mermaid-er-viewer、コマンド: npx -y mermaid-er-viewer-mcp
```

手動で設定する場合は、`~/.claude.json` または プロジェクトの `.mcp.json` に以下を追加：
...
| ツール | 説明 |
|--------|------|
| `parse-diagram` | `.mmd` ファイルをパースしてエンティティ・リレーションシップの JSON を返す |
| `list-mmd-files` | ディレクトリ内の `.mmd` ファイルを一覧する |
| `get-layout` | レイアウトデータ（位置・ラベル）を取得する |
| `save-layout` | レイアウトデータを保存する（日本語ラベルの設定も可能） |
| `auto-layout` | エンティティを自動整列する（sugiyama / force / community） |
| `open-viewer` | ブラウザで ER 図ビューワーを起動する |
| `close-viewer` | 起動中のビューワーを停止する |
